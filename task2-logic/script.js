...
function updateCart() {
  ...
  cartSubtotal.textContent = formatEgp(subtotal);
  shippingCost.textContent = formatEgp(shipping);
  cartTotal.textContent = formatEgp(subtotal + shipping);
}
...
governorate.addEventListener("change", function () {
  ...
  deliveryTime.textContent = selectedOption.dataset.days;
  updateCart();
});

document.querySelector(".checkout-grid form").addEventListener("submit", function (event) {
  event.preventDefault();
